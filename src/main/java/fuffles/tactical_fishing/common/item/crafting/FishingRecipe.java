...
public class FishingRecipe implements Recipe<Container>
{
	...
	@Override
	public ItemStack assemble(Container container, RegistryAccess registry) 
	{
	...
	public static class Serializer implements RecipeSerializer<FishingRecipe>
	{
		...
		{
			if (recipe.getGroup() != null && !recipe.getGroup().isBlank())
				json.addProperty("group", recipe.getGroup());
			json.add("rod", recipe.getRod().toJson());
			json.add("ingredient", recipe.getIngredient().toJson());
			json.add("catch", recipe.getCatch().toJson());
			ResourceLocation result = ForgeRegistries.ITEMS.getKey(recipe.output.getItem());
			json.add("result", GsonUtil.newObject((obj) -> {
				obj.addProperty("item", result.toString());
			}));
			json.add("conditions", GsonUtil.newArray((array) -> {
				array.add(GsonUtil.newObject((obj) -> {
					obj.addProperty("type", ModLoadedCondition.Serializer.INSTANCE.getID().toString());
					obj.addProperty("modid", result.getNamespace());
				}));
			}));
		}
